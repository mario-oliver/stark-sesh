...
/** Method signatures for Subscriptions API; merged onto ApiClient. */
export interface SubscriptionsApi {
  getSubscription(): Promise<ApiResponse<SubscriptionInfo | null>>
  createCheckoutSession(input: CreateCheckoutSessionInput): Promise<ApiResponse<{ url: string; sessionId: string }>>
  syncSubscription(input: SyncSubscriptionInput): Promise<ApiResponse<{ success: boolean; data: SubscriptionInfo }>>
}
