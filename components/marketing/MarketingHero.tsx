...
export function MarketingHero({
  ...
  return (
    <section className={`relative w-full min-h-[min(100vh,1080px)] overflow-hidden ${className}`}>
      ...
      <div className="absolute inset-0 bg-gradient-to-b from-background/35 via-transparent to-background" aria-hidden />
      <div
        className={`relative z-10 max-w-5xl mx-auto px-6 sm:px-8 pt-16 pb-20 sm:pt-20 sm:pb-24 flex flex-col ${alignClass}`}
      >
        {eyebrow && (
          <p className="text-primary/90 text-sm font-medium tracking-widest uppercase mb-4">{eyebrow}</p>
        )}
        <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight text-foreground text-balance mb-6 max-w-4xl">
          {headline}
        </h1>
        {subheadline && (
